...
<template>
  <tr
    class="momotalk-dialogue-tr"
    v-show="screenWidth >= MOBILE_WIDTH && !setting.ui_force_mobile && ML_table"
  >
    <template v-for="idx in i18nDesktopLoopIdx" :key="idx">
      <MessageStudent
        ...
        :dialogue-speaker-translated="
          ML_table[mmt_entry_pos][i18nLangAll[idx]][dialog_entry_pos]['name']
        "
        :dialogue-content-translated="
          ML_table[mmt_entry_pos][i18nLangAll[idx]][dialog_entry_pos]['dialogue']
        "
  ...
  <tr
    ...
    v-show="(screenWidth < MOBILE_WIDTH || setting.ui_force_mobile) && ML_table"
  >
    <MessageStudentMobile
      :dialogueType="data_dialog.MessageCondition"
  ...
  <tr class="momotalk-dialogue-tr" v-if="data_dialog.FavorScheduleId !== 0">
    <td
      :colspan="screenWidth >= MOBILE_WIDTH ? numberOfSelectedLangForDesktop * 2 : 2"
      style="text-align: center; background-color: var(--color-ba-mmt); font-size: 1.2em"
    >
      <b>
        <RouterLink :to="`/scenario/${bond_story_id}`">{{
          $t('comp-mmt-ui-table-view-story')
        }}</RouterLink>
      </b>
    </td>
  </tr>
