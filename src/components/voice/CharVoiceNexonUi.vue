...
<template>
  ...
  <div v-if="!isLoading">
    <KeepAlive>
      ...
      >
        <PvTabList>
          <PvTab value="Normal" v-if="dataVoiceAvailability[0]">{{
            i18n.t('char-voice-ui-select-tab-normal')
          }}</PvTab>
          <PvTab value="Lobby" v-if="dataVoiceAvailability[1]">{{
            i18n.t('char-voice-ui-select-tab-lobby')
          }}</PvTab>
          <PvTab value="Battle" v-if="dataVoiceAvailability[2]">{{
            i18n.t('char-voice-ui-select-tab-battle')
          }}</PvTab>
          <PvTab value="Event" v-if="dataVoiceAvailability[3]">{{
            i18n.t('char-voice-ui-select-tab-event')
          }}</PvTab>
        </PvTabList>
        <PvTabPanels>
          <PvTabPanel value="Normal" v-if="dataVoiceAvailability[0]">
            <component
              :is="CharVoiceNexonNormal"
          ...
          <PvTabPanel value="Lobby" v-if="dataVoiceAvailability[1]">
            <component
              :is="CharVoiceNexonNormal"
              :dataVoice="dataVoice.Lobby!"
              :dataVoiceMt="dataVoiceMt.Lobby!"
            />
          </PvTabPanel>
          <PvTabPanel value="Battle" v-if="dataVoiceAvailability[2]">
            <component
              :is="CharVoiceNexonBattle"
              :dataVoice="dataVoice.Battle!"
              :dataVoiceMt="dataVoiceMt.Battle!"
            />
          </PvTabPanel>
          <PvTabPanel value="Event" v-if="dataVoiceAvailability[3]">
            <component
              :is="CharVoiceNexonEvent"
              :dataVoice="dataVoice.Event!"
              :dataVoiceMt="dataVoiceMt.Event!"
            />
          </PvTabPanel>
        </PvTabPanels>
      </PvTabs>
